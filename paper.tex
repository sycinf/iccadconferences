...
computing substrate (section~\ref{expEval}). Finally, related work is presented in section~\ref{relatedwork}, before we conclude the paper (section~\ref{conclude}).
...
transformed to networks of processes. Using HLS as our backend, they can then be implemented as accelerator networks on FPGA. Compare to accelerators directly synthesized from the original program, and software running on a hard CPU core with much higher clock frequency, our final implementations can achieve significantly better performance.
